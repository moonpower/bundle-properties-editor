...
public class StringUtil {
	...
	/**
	 * Controls and Basic Latin을 제외한 문자를 유니코드로 변환.
	 * 
	 * @param str
	 * @return
	 */
	public static String getUnicodeTextToText(String str) {
		...
		for (int i = 0; i < str.length(); i++) {
			...
			if (!(ch >= 0x0000 && ch <= 0x007f)) {
				sb.append(String.format("\\u%04X", str.codePointAt(i)));
			}
			else {
				sb.append(ch);
			}
	...
	public static String getCamelCase(String source,
			boolean forceHandleFirstCharToCamel, boolean handleNumberToCamel) {
		...
		if (forceHandleFirstCharToCamel) {
			// 첫번째 문자를 대문자로 변경한다.
			source = Character.toUpperCase(source.charAt(0))
		...
		for (int index = 0; index < source.length(); index++) {
			...
			if (Character.isUpperCase(ch)
					|| (handleNumberToCamel && Character.isDigit(ch))) {
				buffer.append(ch);
			}
		}
	...
	public static boolean isNullOrBlank(String currentLogicalName) {
		return currentLogicalName == null
				|| currentLogicalName.trim().isEmpty();
	}
	...
	public static String camelToPlain(String camelString) {
		...
		for (int i = 0; i < camelString.length(); i++) {
			...
			if (prev != null && Character.isLowerCase(prev)
